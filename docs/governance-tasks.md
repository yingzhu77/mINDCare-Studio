...
| 任务 5：心理健康 AI 产品安全边界 | ⏳ 待执行 | — | 无前置依赖 |
| 任务 6：CLAUDE.md 精简 + AI 交接模板 | ⏳ 待执行 | — | 无前置依赖（`docs/ai-handoff.md` 和 `docs/current-state.md` 已创建但尚未从 CLAUDE.md 分离） |
| 任务 7：修复 Vite 代理 SSE 缓冲 | ⏳ 待执行 | — | 无前置依赖 |
...
1. 每个任务开一个新对话窗口（或 /clear 后执行）
2. 复制对应提示词，粘贴到新窗口
3. 执行完毕后，更新 `docs/ai-handoff.md` 记录完成状态
4. 任务 1-4 建议顺序执行（有依赖关系）
5. 任务 5-7 可并行分派给 3 个窗口
